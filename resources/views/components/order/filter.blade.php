...
<form name="filter" method="post" action="{{ route('order.indexPost') }}">
    ...
    <div class="border-b flex flex-wrap sm:justify-end gap-4 m-2 pb-2">
        ...
        <div class="flex flex-nowrap justify-between w-full sm:w-min">
            <label>
                <input class="border rounded-xl" type="date" name="date-start" value="{{ $filter['date-start'] ?? '' }}" />
            </label>
            <span class="p-2">-</span>
            <label>
                <input class="border rounded-xl" type="date" name="date-end" value="{{ $filter['date-end'] ?? '' }}" />
            </label>
        </div>
